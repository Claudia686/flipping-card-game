...
describe('FlippingCardGame', () => {
    ...
    beforeEach(async () => {
        [deployer, player, player1, player2, winner, loser] = await ethers.getSigners()
        ...
        // const tx1 = await flippingCardGame.getSubscription(subscriptionId)
    ...
    describe('Deployment', () => {
        ...
        it('Should have correct VRFCoordinator address', async () => {
            ...
            expect(storedCoordinatorAddress.toLowerCase()).to.equal(vrfCoordinatorV2_5Mock.target.toLowerCase());
        })
    ...
    describe('Distribute prize', () => {
        let fee, requestId, gameId

        beforeEach(async () => {
            // owner create a game
            fee = await ethers.parseEther("2");
            await flippingCardGame.connect(deployer).createGame(fee);

            // user joins the game
            await flippingCardGame.connect(player1).joinGame();
            await flippingCardGame.connect(player2).joinGame();
            ...
            const request = await flippingCardGame.requestInProgress()
            ...
            // const requestInProgress = await flippingCardGame.requestInProgress()
            // expect(requestInProgress).to.equal(false)
        ...
        it("Should distribute the prize", async () => {
            await flippingCardGame.gameId()
            gameId = 0;
            const getPlayers = await flippingCardGame.getGamePlayers(gameId)
            expect(getPlayers.length).to.equal(2)
            expect(getPlayers[0]).to.equal(player1.address)
            expect(getPlayers[1]).to.equal(player2.address)
            console.log("Players in the game", getPlayers)

            // await flippingCardGame.connect(deployer).stopGame();
            // const winner = await flippingCardGame.winner()
            // const loser = await flippingCardGame.loser()

            console.log("Winner address is", winner.address)
            console.log("Loser address is", loser.address)
            console.log("player1 address is", player1.address)
            console.log("player2 address is", player2.address)
            


            await flippingCardGame.connect(winner).distributePrize();
